...
export default function HomePage() {
  ...
  const handleDownload = async (videoId: string, title: string) => {
    ...
    try {
      ...
      setDownloadStates((prev) => ({ ...prev, [videoId]: 'error' }));
      setTimeout(() => {
        setDownloadStates((prev) => ({ ...prev, [videoId]: 'idle' }));
      }, 3000);
    }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sw-logo { font-size: 18px; font-weight: 800; letter-spacing: -0.02em; color: #fff; }
        @media (min-width: 640px) { .sw-logo { font-size: 22px; } }
        .sw-logo span { color: #1877f2; }
        .sw-logo-underscore {
          display: block; width: 24px; height: 3px;
          background: #1877f2; margin-top: 2px; border-radius: 2px;
        }
        ...
        .sw-hero-label {
          ...
          color: #fff; background: #1877f2; padding: 4px 12px;
          border-radius: 2px; margin-bottom: 18px;
        }
        @media (min-width: 640px) { .sw-hero-label { font-size: 11px; padding: 5px 14px; margin-bottom: 24px; } }
        ...
        .sw-search-btn {
          ...
          letter-spacing: 0.04em; padding: 10px 12px; border-radius: 4px;
        ...
        @media (min-width: 480px) { .sw-search-btn { padding: 11px 22px; } }
        ...
        @media (min-width: 480px) { .sw-features { grid-template-columns: repeat(3, 1fr); } }

        .sw-feat {
          padding: 16px; background: #111827;
          border: 1px solid rgba(255,255,255,0.06);
          border-radius: 6px; border-left: 3px solid #1877f2; transition: background 0.15s;
        }
        ...
        .sw-skeleton {
          height: 70px; border-radius: 6px; background: #111827;
          border: 1px solid rgba(255,255,255,0.06);
        ...
        .sw-skeleton::after {
          ...
          background-size: 200% 100%; animation: sw-shimmer 1.6s infinite;
        }
        ...
        @media (min-width: 480px) { .sw-card { gap: 14px; padding: 10px 14px; } }
        ...
        .sw-thumb {
          position: relative; width: 52px; height: 38px;
          border-radius: 4px; overflow: hidden; flex-shrink: 0; background: #1a2234;
        }
        @media (min-width: 480px) { .sw-thumb { width: 70px; height: 50px; } }
        ...
        @media (min-width: 480px) { .sw-thumb-duration { font-size: 9px; bottom: 3px; right: 3px; } }
        ...
        @media (min-width: 480px) { .sw-dl-btn { padding: 7px 14px; font-size: 12px; } }
        .sw-dl-btn:hover { background: #1877f2; border-color: #1877f2; color: #fff; }
        .sw-dl-btn:disabled { opacity: 0.35; cursor: default; }
        .sw-dl-btn.error { border-color: rgba(239,68,68,0.4); background: rgba(239,68,68,0.08); color: #ef4444; }

        .sw-dl-label { display: none; }
        @media (min-width: 480px) { .sw-dl-label { display: inline; } }
        ...
        .sw-footer {
          ...
          text-transform: uppercase; color: rgba(255,255,255,0.15); text-align: center;
        }
        @media (min-width: 640px) { .sw-footer { padding: 20px 40px; font-size: 11px; } }
      `}</style>
            ...
            <div className="sw-search-box">
              ...
              >
                {loading
                  ...
                  : <><Search size={14} /><span className="sw-btn-label">{t('hero.button')}</span></>
                }
              </button>
            </div>
          </div>
            ...
            <div className="sw-results">
              {loading ? (
                ...
                <>
                  ...
                  {results.map((r, i) => {
                    const dlState = downloadStates[r.id] || 'idle';
                    return (
                      <div
                        key={r.id}
                        className="sw-card"
                      ...
                      >
                        ...
                        <button
                          className={`sw-dl-btn${dlState === 'error' ? ' error' : ''}`}
                          disabled={dlState === 'pending'}
                          onClick={() => handleDownload(r.id, r.title)}
                        >
                          {dlState === 'pending' && <span className="sw-spin"><Loader2 size={13} /></span>}
                          {dlState === 'idle' && <><Download size={13} /><span className="sw-dl-label">{t('results.download')}</span></>}
                          {dlState === 'error' && <><Download size={13} /><span className="sw-dl-label">Erreur</span></>}
                        </button>
                      </div>
                    );
                  })}
